...
namespace TheEndTimes_Beastmen
{
    public class QuestNode_Root_WandererJoin_WalkIn : QuestNode_Root_WandererJoin
    {
        ...
        public override Pawn GeneratePawn()
        {
            ...
            request = new PawnGenerationRequest(pawnKindDef, (Faction)null, PawnGenerationContext.PlayerStarter, -1, true, false, false, true, false, RelationWithColonistWeight, false, true, true, true, true, false, false, false, false, 0.0f, 0.0f, (Pawn)null, 1f, (Predicate<Pawn>)null, (Predicate<Pawn>)null, (IEnumerable<TraitDef>)null, (IEnumerable<TraitDef>)null, new float?(), new float?(), new float?(), fixedGender);
        ...
        private PawnKindDef DeterminePawnKind ()
        {
            ...
            if (!hasBray || rando < 3)
            {
                return PawnKindDef.Named("RH_TET_Beastmen_BrayShaman");
            }
            else if (rando < 5)
            {
                return PawnKindDef.Named("RH_TET_Beastmen_GreatBrayShaman");
            }
            else if (rando < 25)
            {
                return PawnKindDef.Named("RH_TET_Beastmen_Minotaur");
            }
            else if (rando < 45)
            {
                return PawnKindDef.Named("RH_TET_Beastmen_Wargor");
            }
            else if (rando < 60)
            {
                return PawnKindDef.Named("RH_TET_Beastmen_Ravager");
            }
            else if (rando < 75)
            {
                return PawnKindDef.Named("RH_TET_Beastmen_Raider");
            }
            else if (rando < 95)
            {
                return PawnKindDef.Named("RH_TET_Beastmen_RaiderMelee");
            }

            return PawnKindDef.Named("RH_TET_Beastmen_CygorBeastmenPlayer");
        }
