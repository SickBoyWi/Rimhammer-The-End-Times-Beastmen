...
{
    public class Graphic_FlickerMulti : Graphic_Collection
    {
          ...
          float extraRotation)
        {
            ...
            else
            {
                ...
                if (thing != null) { 
                    ticksGame += Mathf.Abs((thing.thingIDNumber + RH_TET_BeastmenMod.random.Next(1, 1000) ) ^ 8453458);
                }
                float num2 = 1f;
                CompProperties_FireOverlayMulti propertiesFireOverlay = (CompProperties_FireOverlayMulti)null;
                Fire fire = thing as Fire;
                if (fire != null)
                    num2 = fire.fireSize;
                else if (thingDef != null)
                {
                    propertiesFireOverlay = thingDef.GetCompProperties<CompProperties_FireOverlayMulti>();
                    if (propertiesFireOverlay != null)
                    { 
                        num2 = propertiesFireOverlay.fireSize;
                    }
                }
                int num1 = ticksGame / 23;
