...
{
    public class Graphic_FlickerCustom2 : Graphic_Collection
    {
          ...
          float extraRotation)
        {
            ...
            else
            {
                ...
                float num2 = 1f;
                CompProperties_FireOverlayEvenMore propertiesFireOverlay = (CompProperties_FireOverlayEvenMore)null;
                Fire fire = thing as Fire;
                if (fire != null)
                    num2 = fire.fireSize;
                else if (thingDef != null)
                {
                    propertiesFireOverlay = thingDef.GetCompProperties<CompProperties_FireOverlayEvenMore>();
                    if (propertiesFireOverlay != null)
                    { 
                        num2 = propertiesFireOverlay.fireSize;
                    }
                }
                int num1 = ticksGame / 7;
